...
all: build
	@echo "bloombox-apothecary is ready."
...
ifeq ($(BUILDBOT),yes)
test:
	wct --job-name "bloombox-apothecary" --expanded --build-number "$(BUILD_NUMBER)" --sauce-tunnel-id "$(SAUCE_TUNNEL)" --plugin wct-jenkins
else
test:
	@wct --job-name "bloombox-apothecary"
endif
